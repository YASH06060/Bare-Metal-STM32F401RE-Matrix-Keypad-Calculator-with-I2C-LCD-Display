...
typedef struct {
    uint8_t pinNumber;
    uint8_t mode;        // GPIO_MODE_IN=0, GPIO_MODE_OUT=1, etc
    uint8_t speed;       // Output speed config
    uint8_t outputType;  // Push-pull, open drain
    uint8_t pull;        // None, pull-up, pull-down
    uint8_t altFuncSel;  // Alternate function number (if mode == AF)
} GPIO_Config_Mod_t;
